...
func TestTransactor(t *testing.T) {
	...
	t.Run("it should implement the Transactor interface", func(t *testing.T) {
		...
		assert.Implements(t, (*transactor.Transactor)(nil), sqlxTransactor.FakeTransactor{})
	})

	t.Run("the fake transactor should not be within a transaction", func(t *testing.T) {
		t.Parallel()

		assert.False(t, sqlxTransactor.FakeTransactor{}.IsWithinTransaction(context.Background()))
	})
